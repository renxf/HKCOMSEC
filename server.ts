...
Response MUST be just the JSON array.`;
  ...
  app.post("/api/rpa/search", async (req, res) => {
    ...
    try {
      // Use Direct HK CR Open Data API Endpoint Instead of Web Scraping
      ...
      if (queryChi) {
          ...
          const [localR, foreignR] = await Promise.all([fetchNames(localUrlChi), fetchNames(foreignUrlChi)]);
          conflicts.push(...localR, ...foreignR);
      }
